...
import type { SentenceCompletionItem } from '@/types/content';
...
type Phase = 'choosing' | 'chosen' | 'recording' | 'assessing' | 'review';
...
export default function SentenceCompletionActivity({
  ...
  const handleStopRecording = useCallback(async () => {
    ...
    if (studentId && activityId) {
      try {
        savedUri = await keepRecording(studentId, activityId, itemIndex, uri);
      } catch { savedUri = uri; }
    }
    ...
    setPhase('review');
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [studentId, activityId, itemIndex, whisperCtx, vadCtx, current]);
  ...
  // ── Retry current item (re-record only) ──────────────────────────────────────
  ...
  const handleNext = useCallback((passed: boolean) => {
    soundRef.current?.unloadAsync().then(() => { soundRef.current = null; });
    const nextScores = [...itemScores.slice(0, itemIndex), passed];
    ...
    <View className="bg-white rounded-2xl border border-border p-5 mb-3">
      ...
      <View className="rounded-xl p-4 mb-4" style={{ backgroundColor: accentColor + '10' }}>
        <Text className="text-xs font-semibold mb-2" style={{ color: accentColor }}>
          Choose the correct word and say the sentence:
        </Text>
      ...
      {/* ── CHOOSING / CHOSEN PHASE: choice buttons ──────────────────────────── */}
      {(phase === 'choosing' || phase === 'chosen') && (
        <View className="gap-2 mb-4">
          {current.choices.map((choice, i) => {
            const isSelected = selectedIndex === i;
            const isCorrect = i === current.correctIndex;
            const locked = phase === 'chosen';
            ...
            return (
              <Pressable
                ...
                disabled={locked}
                className="rounded-xl px-4 py-3.5 border flex-row items-center justify-between active:opacity-70"
      ...
      {/* ── CHOSEN PHASE: full sentence reveal + record button ───────────────── */}
      {phase === 'chosen' && (
        <>
          {/* Full sentence reveal */}
          <View className="rounded-xl p-3 mb-4" style={{ backgroundColor: '#10B98110', borderWidth: 1, borderColor: '#10B98140' }}>
            <Text className="text-xs font-semibold mb-1" style={{ color: '#059669' }}>
              Full sentence:
            </Text>
          ...
          >
            ...
            <Text className="text-white font-bold text-base">Record Sentence</Text>
          </Pressable>
        </>
      )}
      ...
      {phase === 'recording' && (
        <View className="gap-4">
          <View className="items-center gap-3 py-4">
            ...
            <Text className="text-xs text-text-muted">Speak the full sentence clearly</Text>
          </View>
        ...
        <View className="gap-3">
          ...
          >
            <View className="flex-row items-center gap-2">
              ...
              >
                {completionResult.passed ? 'Sentence spoken correctly!' : 'Sentence incomplete'}
              </Text>
            </View>
          ...
          <View className="flex-row gap-2 mt-1">
            ...
            >
              <Text className="text-white font-bold text-sm">
                {itemIndex + 1 >= items.length ? 'Finish' : 'Next →'}
              </Text>
            </Pressable>
          </View>
        </View>
